...
[employee_id] => C225 [employee_name] => Ranya Nigam [gender] => Female [designation] => Assistant Manager [email] => ranya.nigam@shopclues.com [mobile_number] => 9711038099 [address] => Near Sarai Khawaja, Faridabad 121003 [number_of_seats] => 4 [vehicle_number] => HR 12 3562 [latitude] => 28.4999 [longitude] => 77.3021 [zone_id] => 1 [owns_vehicle] => 1 )
    ...
    public function storeRequest($driver_id, $passenger_id, $notify, $message){

        $stmt = $this->conn->prepare("INSERT INTO request(driver_id, passenger_id, notify, message, created_at) VALUES(?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssss", $driver_id, $passenger_id, $notify, $message);
        $result = $stmt->execute();
        $stmt->close();
 
        // check for successful store
        if ($result) {
            $stmt = $this->conn->prepare("SELECT * FROM request WHERE sender_id = ? AND receiver_id = ?");
